...
func getEvent(eventID, loginUserID int64) (*Event, error) {
	...
	reservationMap := make(map[int64]Reservation)
	...
	for rows.Next() {
		...
		reservationMap[reservation.SheetID] = reservation
	}

	for _, rank := range []string{"S", "A", "B", "C"} {
		for _, s := range sheetMap[rank] {
			sheet := s
			event.Sheets[sheet.Rank].Price = event.Price + sheet.Price
			event.Total++
			event.Sheets[sheet.Rank].Total++

			reservation, ok := reservationMap[sheet.ID]
...
func main() {
	...
	e.GET("/initialize", func(c echo.Context) error {
		...
		for rows.Next() {
			...
			sheetMap[sheet.Rank] = append(sheetMap[sheet.Rank], sheet)
		}
